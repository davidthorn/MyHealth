...
@MainActor
public final class CaloriesSummaryService: CaloriesSummaryServiceProtocol {
    ...
    public func updates() -> AsyncStream<CaloriesSummaryUpdate> {
        AsyncStream { continuation in
            let task = Task { [healthKitAdapter] in
                for await summary in healthKitAdapter.activeEnergySummaryStream(days: 14) {
                    guard !Task.isCancelled else { return }
                    continuation.yield(CaloriesSummaryUpdate(summary: summary))
                }
                continuation.finish()
            }

//
//  StandHoursSummaryService.swift
//  MyHealth
//
//  Created by Codex.
//
...
@MainActor
public final class StandHoursSummaryService: StandHoursSummaryServiceProtocol {
    ...
    public func updates() -> AsyncStream<StandHoursSummaryUpdate> {
        AsyncStream { continuation in
            let task = Task { [healthKitAdapter] in
                for await summary in healthKitAdapter.standHoursSummaryStream(days: 14) {
                    guard !Task.isCancelled else { return }
                    continuation.yield(StandHoursSummaryUpdate(summary: summary))
                }
                continuation.finish()
            }
